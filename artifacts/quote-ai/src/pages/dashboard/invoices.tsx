        ...
        <>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex gap-1 p-1 bg-muted rounded-full w-fit overflow-x-auto max-w-full">
              {FILTERS.map((f) => (
                <button key={f} onClick={() => setFilter(f)} className={cn("px-3 py-1.5 text-sm font-medium rounded-full transition-all whitespace-nowrap", filter === f ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}>
                  {t(`invoices.filter.${f}`)}
                </button>
              ))}
            </div>
          ...
          {isLoading ? (
            <div className="space-y-2">{[0, 1, 2].map((i) => <Skeleton key={i} className="h-16 w-full rounded-[var(--radius)]" />)}</div>
          ) : items.length === 0 ? (
            <div className="rounded-[var(--radius)] border border-slate-200 bg-card p-12 text-center">
