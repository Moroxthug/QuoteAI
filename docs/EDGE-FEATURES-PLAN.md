...
Gated behind a new dedicated `meta_lead_ads` plan feature (Elite), not reusing `wave_sync` — every other integration in this plan got its own feature flag, so this does too. Google Local Services Ads half was **not built** at the time — lower priority, narrower applicability. It has since been scaffolded as Phase 29 (below), engineering-track, gated on Google's own approval process.
...
## 15a. Phase 29 — Google Local Services Ads lead capture (engineering track) — **scaffolded & pushed (2026-09-15)**

**Built**: the Google half of Phase 28's ad-lead-capture plan, deferred at the time because — unlike Meta's Graph API — Google LSA leads aren't reachable through a simple public API. They're only queryable via the **Google Ads API**'s `local_services_lead` resource using GAQL (Google Ads Query Language), which needs three things Meta's flow didn't: a developer token (`GOOGLE_ADS_DEVELOPER_TOKEN`, Google's own approval gate, not self-serve), a manager/MCC account (`GOOGLE_ADS_LOGIN_CUSTOMER_ID`, QuoteAI's own Google Ads manager account each customer's LSA account links under), and no webhook/push mechanism at all — leads must be **polled**. Same partner/approval-gated profile as Phase 16 (Financeit) and Phase 27 (Flinks): the full scaffold is built now so it goes live the moment the developer token and manager account are approved, and it no-ops safely (`google_lsa_connections` empty → the polling sweep checks zero connections) until then.

Built exactly like the equivalent scaffolds: `artifacts/api-server/src/lib/googleLsaClient.ts` wraps a dedicated OAuth2 app (scope `.../auth/adwords`, same `accounts.google.com` dialog pattern as `googleCalendarClient.ts` but its own client id/secret/consent screen — different scope needs different app registration) plus a GAQL `searchStream` POST against `googleads.googleapis.com` carrying the developer-token and login-customer-id headers. New tables `google_lsa_connections` (userId PK, encrypted refresh token, `lsaCustomerId`, `lastPolledAt`/`lastLeadAt`) and `google_lsa_import_log` (append-only, mirrors `meta_lead_ads_import_log`) — migration `0031_phase29_google_lsa.sql`, **not applied live** (no real developer token exists yet to test against). `leads` gained nullable `google_lsa_lead_id/lead_type/category` attribution columns; `LEAD_SOURCES`/`LEAD_CONSENT_SOURCES` gained a `"google_lsa"` value (consent basis: the customer's own contact through Google's LSA platform, same reasoning as `meta_lead_ads`).

Because there's no webhook, `artifacts/api-server/src/googleLsa/maintenance.ts`'s `runGoogleLsaPollCheck()` is wired into the existing cron tick (`routes/cron.ts`) exactly like `flinks/maintenance.ts`'s `runFlinksSyncCheck` — it sweeps every enabled connection, polls GAQL for leads created since `lastPolledAt`, and catches per-connection errors so one bad connection can't kill the sweep. OAuth's `state` param has to carry the customer's chosen LSA account id through the round trip (Google's callback only echoes back `code`/`state`, and Vercel serverless functions share no memory between `/connect` and `/callback`), so the signed HMAC state token bundles `userId + lsaCustomerId + nonce + issued-at`, one field more than Meta/Wave/QuickBooks's `userId`-only state. The Settings UI (`GoogleLsaTab`/`GoogleLsaImportLogCard` in `settings.tsx`) collects the LSA account id in a text field before redirecting to Google, mirroring `MetaLeadAdsTab`'s card/badge/import-log layout. Gated behind a new `google_lsa` Elite plan feature, separate from `meta_lead_ads`.

**Known scope limit**: the `local_services_lead` resource itself carries no contact details (name/phone/email) — those live on a separate linked `local_services_lead_conversation` resource (transcript/phone number). The scaffold imports the lead shell (id, type, category, timestamp) now; wiring the conversation lookup for full contact details is a small follow-up once developer-token access is actually live and there's a real account to test the linked-resource query against.

**Feasibility**: same as Financeit/Flinks — this is a business-development gate (apply for the developer token, stand up a manager account), not an engineering one. The code is ready; nothing runs until Google approves.
