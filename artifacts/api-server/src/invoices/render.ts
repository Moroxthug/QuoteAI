...
.inv h1, .inv h2.doc-title { font-size:24px; margin:0 0 2px; letter-spacing:-0.01em; }
...
.inv table.totals td, .inv table.totals th { padding:5px 8px; font-weight:inherit; text-align:left; }
.inv table.totals th { color:inherit; }
.inv table.totals tr.total td, .inv table.totals tr.total th { font-weight:800; font-size:16px; border-top:2px solid #111827; background:#f9fafb; }
.inv table.totals tr.muted td, .inv table.totals tr.muted th { color:#6b7280; }
.inv table.totals tr.balance td, .inv table.totals tr.balance th { font-weight:700; color:#047857; }
.inv .box { background:#f5f3ff; border:1px solid #ede9fe; border-radius:10px; padding:12px 16px; margin:20px 0 8px; font-size:13px; }
.inv .box h2, .inv .box h3 { margin:0 0 6px; font-size:12px; text-transform:uppercase; letter-spacing:0.06em; color:#5b21b6; }
...
/**
 * `embedded` renders the document inside a page that already has an <h1>
 * (the dashboard invoice detail): the document title becomes an <h2> so the
 * page keeps exactly one first-level heading (Phase 83). The public page and
 * the PDF are the document itself and keep their <h1>.
 */
export function renderInvoiceHtml(inv: Invoice, payments: InvoicePayment[] = [], opts: { embedded?: boolean } = {}): string {
  const lang = inv.language as Lang;
  const credit = isCreditNote(inv);
  const parts: string[] = [];
  // One level below the document title, whatever that title is here: <h1> on
  // the public page and in the PDF, <h2> when embedded in a dashboard page
  // that has its own <h1> (Phase 83).
  const sub = opts.embedded ? "h3" : "h2";
  const wm = watermark(inv);
  if (wm) parts.push(`<div class="wm ${inv.status}">${ti(wm, lang)}</div>`);
  parts.push(opts.embedded ? `<h2 class="doc-title">${esc(invoiceTitle(inv, lang))}</h2>` : `<h1>${esc(invoiceTitle(inv, lang))}</h1>`);
  ...
  const totals: string[] = [];
  totals.push(`<tr><th scope="row">${ti("subtotal", lang)}</th><td class="num">${fmtCents(inv.subtotalCents, lang)}</td></tr>`);
  if (inv.holdbackCents > 0) {
    totals.push(`<tr><th scope="row">${ti("holdback", lang)} (${inv.holdbackPercent}%)</th><td class="num">− ${fmtCents(inv.holdbackCents, lang)}</td></tr>`);
    totals.push(`<tr class="muted"><th scope="row">${ti("taxable", lang)}</th><td class="num">${fmtCents(inv.taxableCents, lang)}</td></tr>`);
  }
  for (const t of inv.taxLines) {
    totals.push(`<tr><th scope="row">${esc(taxLabel(t.label, lang))} ${t.rate}%${t.registrationNumber ? ` <span style="color:#6b7280">(${esc(t.registrationNumber)})</span>` : ""}</th><td class="num">${fmtCents(t.amountCents, lang)}</td></tr>`);
  }
  totals.push(`<tr class="total"><th scope="row">${credit ? ti("creditTotal", lang) : ti("total", lang)}</th><td class="num">${fmtCents(inv.totalCents, lang)}</td></tr>`);
  if (!credit && inv.paidCents > 0 && inv.status !== "void") {
    totals.push(`<tr class="muted"><th scope="row">${ti("paid", lang)}</th><td class="num">− ${fmtCents(inv.paidCents, lang)}</td></tr>`);
    totals.push(`<tr class="balance"><th scope="row">${ti("balance", lang)}</th><td class="num">${fmtCents(Math.max(0, inv.totalCents - inv.paidCents), lang)}</td></tr>`);
  }
  ...
  if (!credit && inv.status !== "void" && inv.status !== "paid") {
    ...
    if (lines.length) parts.push(`<div class="box"><${sub}>${ti("payment", lang)}</${sub}>${lines.join("")}<div class="note">${ti("reference", lang)}</div></div>`);
  }
  ...
  if (payments.length > 0 && !credit) {
    ...
    parts.push(`<${sub} style="font-size:12px;text-transform:uppercase;letter-spacing:0.06em;color:#6b7280;margin:16px 0 4px">${ti("paymentsReceived", lang)}</${sub}><table class="grid"><tbody>${prow}</tbody></table>`);
  }
