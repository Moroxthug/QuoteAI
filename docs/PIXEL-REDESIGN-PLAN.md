...
- **Phase 47 — Notifications page & Archive, real backend (decided 2026-09-16: build it for real).**
  - **Notifications page**: mostly UI. The backend already exists — `notificationsTable` (`lib/db/src/schema/automation.ts`), `GET /api/notifications` (supports `limit` up to 100, returns `{items, unread}`), `POST /api/notifications/read` (all or by id). No new schema needed; a dedicated page just needs a higher-limit fetch (add simple cursor/offset pagination only if 100 proves too few in practice) and the `.n-row`/`.n-dot` markup from the mockup.
  - **Archive**: real backend work, nothing to reuse. No table has an `archivedAt`/soft-delete concept today. Scope: add a nullable `archivedAt` timestamp to quotes, clients, invoices, jobs, and contracts; add archive + restore endpoints (per-entity or one small shared service); add a unified `GET /api/archive` that unions across those tables sorted by `archivedAt desc`; and — important, easy to miss — update every existing list endpoint for those 5 entities to filter out archived rows by default, plus add the actual "Archive" action somewhere in each entity's detail/row UI (the mockup's Archive page only shows already-archived rows with a Restore action; something has to put rows there).
