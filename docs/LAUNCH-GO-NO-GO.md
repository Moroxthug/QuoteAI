...
| Real-device pass | Still emulated only, but Phase 82 added the phone-gutter check to `qa:visual`: at 375 px every element with its own text must keep 12 px from both edges, measured on the glyphs. It found the homepage hero, all eight CTA bands and the footer bottom row flush against the glass, and `/pricing` scrolling sideways. A real iPhone Safari / Android Chrome pass and a screen-reader pass are still owed. | 30 minutes on each: sign-up → quote → send → `/p` accept → `/sign` on the phone → `/t` clock-in with location | Owner |
