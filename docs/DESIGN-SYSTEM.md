...
## Phase 37 (settings/billing/team/catalog/documents/profile/onboarding/imports) — built 2026-09-16

Presentation-only pass, same mechanical sweep as Phases 35/36, across the last 10 dashboard-area pages.

- **`settings.tsx`** (~2950 lines, 9 tabs): bumped the `<h1>` from plain `text-3xl font-bold` to the standard extrabold + `Settings` icon + `text-slate-500` subtitle pattern. Swept every custom container to `rounded-[var(--radius)]` (all `rounded-2xl` loading skeletons, plus panels/banners/code blocks/upsell tiles that were `rounded-xl`) and every small nested element (icon-avatar squares, inline warning/info banners, key/webhook rows, tx rows) to `rounded-[var(--radius-sm)]` (previously a mix of `rounded-xl`/`rounded-lg`). The tab-navigation strip at the bottom of the file (`flex gap-1 p-1 bg-muted`, a custom segmented control, not the `Tabs` primitive) went from `rounded-xl`/`rounded-lg` to `rounded-full` on both wrapper and buttons, matching the pill-toggle convention from `tabs.tsx`/Phase 36's analytics period switcher.
- **`settings-business-tab.tsx`**: loading skeletons to `rounded-[var(--radius)]`; the 5 switch/setting rows (`rounded-lg border px-4 py-3`) to `rounded-[var(--radius-sm)]` as nested elements.
- **`settings-security-tab.tsx`**: loading skeletons to `rounded-[var(--radius)]`; the TOTP-URI banner, backup-codes grid, and per-session rows (`rounded-xl`) to `rounded-[var(--radius-sm)]` as nested elements.
- **`billing.tsx`**: header bumped to extrabold + `CreditCard` icon + `text-slate-500` subtitle. Icon-avatar squares to `rounded-[var(--radius-sm)]`; all other panels (plan card sections, no-sub state, quota/feature boxes, warning banners) to `rounded-[var(--radius)]`.
- **`team.tsx`**: header already matched. Fixed the tab-strip (`rounded-xl`/`rounded-lg` → `rounded-full`, same pill convention as settings.tsx), the equipment-row icon square to `rounded-[var(--radius-sm)]`, and all list/empty-state/filter-bar containers (`rounded-2xl`) to `rounded-[var(--radius)]`.
- **`catalog.tsx`**: header bumped to extrabold + `BookOpen` icon + `text-slate-500` subtitle. Fixed the OCR-import preview list container (`rounded-lg`) to `rounded-[var(--radius-sm)]`.
- **`documents.tsx`**: this page predates i18n and still has plain English copy — left as-is (same call as `clients/index.tsx` in Phase 35). Header bumped to extrabold + `FolderOpen` icon + `text-slate-500` subtitle. Document-row cards and the upload dropzone (`rounded-xl`) to `rounded-[var(--radius)]`; icon-avatar squares and nested rows (price-alert rows, comparison rows, expanded-items table, price-tile grid) to `rounded-[var(--radius-sm)]`.
- **`profile.tsx`**: a near-duplicate of `settings.tsx`'s `AccountTab` on its own route. Header bumped to extrabold + `User` icon + `text-slate-500` subtitle; logo-preview box (`rounded-lg`) to `rounded-[var(--radius-sm)]`.
- **`onboarding.tsx`**: full-screen wizard with its own deliberate centered-card design (distinct from the sidebar dashboard chrome) — left the step titles (`text-2xl font-bold text-gray-900`) as the intentional exception, same category as `home.tsx`'s different treatment in Phase 33. Fixed 2 leftover hardcoded `rgba(124,58,237,...)`/`rgba(6,182,212,...)` (violet/cyan) inline gradient backgrounds on the step-1 and step-2 welcome icons — the same category of miss as Phase 33's `home.tsx` and Phase 36's dashboard/assistant fixes — to `rgba(16,16,49,...)`/`rgba(15,151,162,...)` (navy/teal). Bumped the welcome-icon circles and the two step cards (`rounded-2xl`) to `rounded-[var(--radius)]`, and the logo-preview box (`rounded-lg`) to `rounded-[var(--radius-sm)]`.
- **`imports/index.tsx`**: header already matched. Swept the candidate-review card, both upload-dropzone panels, the batches panel, and the empty-queue state (`rounded-lg`/`rounded-xl`) to `rounded-[var(--radius)]`.

No hardcoded violet/indigo/cyan Tailwind classes found in any of these 10 files (Phase 31 already covered them); the only inline-`style` color miss was in `onboarding.tsx`, fixed above.

Verification: `npm run typecheck` (tsc, no emit) is clean. No dev-server visual check — same environment limitation as prior dashboard-only phases (backend `/api/*` proxy unreachable here). This was the last page-sweep phase in the visual-redesign-plan's punch list.
