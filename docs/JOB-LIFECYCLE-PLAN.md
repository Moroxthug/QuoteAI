...
### Phase 4 — Invoicing ✅ (built 2026-09-13, migration 0005 applied)

**Schema** (`lib/db/src/schema/invoices.ts`): `invoices` (project/client/contract/milestone links, `payment_term_id`, `credit_note_for_id`, number, type deposit|progress|final|holdback_release|change_order|manual|credit_note, status draft|sent|viewed|partially_paid|paid|overdue|void, language, province, issue/due dates, `scheduled_for` + `scheduled_notified_at` for lien-period releases, contractor + customer party snapshots incl. GST/HST/QST/PST numbers, lines jsonb, subtotal / holdback % + cents / taxable / tax lines (with the registration number per component) / total / paid cents, payment instructions snapshot, hashed public token, PDF url + hash, `auto_send_at`, reminder count). `invoice_payments` (method incl. `credit_note`, `credit_note_id`), `invoice_events`, `invoice_sequences` (user × year × kind → `INV-2026-0042` / `CN-2026-0003`). Partial unique indexes make the automations idempotent: one live invoice per (job, payment term) and one live final / holdback release per job. `AutomationSettings` gained `invoiceAutoSendAfterHours` and `invoiceReminders`. Migration `lib/db/drizzle/0005_phase4_invoices.sql` (additive, idempotent).

**Math** (`artifacts/api-server/src/invoices/math.ts`, pure, unit-run in `math.test.ts`): holdback is withheld on the pre-tax value and tax is charged on `subtotal − holdback` (CRA: tax on the holdback becomes payable at release), so the release invoice bills the withheld amounts + tax. Percent terms apply to the contract's pre-tax subtotal; fixed terms (incl. tax in the schedule) are backed out with the contract rate. The final invoice bills `job pre-tax value (contract + signed change orders) − pre-tax already invoiced`, never negative — rounding and change orders reconcile there. Lien periods: BC 55 d, ON/AB 60 d, others 60 d. `statusAfterPayment`, `arAging` (current / 1–30 / 31–60 / 61–90 / 90+).

**Service** (`invoices/service.ts`): `buildInvoiceContext` (parties from profile + client + contract variables, province, language QC→FR, e-transfer / cheque instructions), `createInvoice`, `draftDepositInvoice` / `draftMilestoneInvoice` (a completion-type term linked to a milestone becomes the final invoice) / `draftFinalInvoice` / `draftHoldbackReleaseInvoice` (scheduled `completedAt + lien days`), `applyAutoSendPolicy` (send now · arm review-then-auto-send timer · leave draft · scheduled release; one notification), `sendInvoice` (dates the draft on first send, renders + stores the PDF under `invoices/<user>/<id>/`, emails PDF + link, resend allowed), `recordPayment` (+ optional receipt email), `removePayment`, `voidInvoice` (refused while real payments exist; voiding a credit note un-applies it), `createCreditNote` (negative numbered document applied to the original as a `credit_note` payment capped at the balance, emailed). Public links are deterministic HMAC tokens (`INVOICE_LINK_SECRET` → `BETTER_AUTH_SECRET` fallback) so the link in the first email survives reminders and receipts; only the hash is stored. `render.ts` (bilingual labels + HTML shared by the dashboard, public page and email) and `pdf.ts` (pdfmake, Letter, DRAFT/PAID/OVERDUE/VOID stamp, tax lines with registration numbers, holdback note, payment box, payments received).

**Automations**: `contract.signed` drafts the deposit invoice and folds its number / outcome into the single "signed — job set up" notification; `milestone.completed` drafts the progress invoice for the released term; `job.completed` drafts the final invoice and, when the contract carries a holdback, the release invoice scheduled for the end of the lien period; `invoice.overdue` posts one in-app notification. **Cron** (`invoices/maintenance.ts`, in `/api/cron/tick`): open invoices past due → overdue; reminder emails 3 / 7 / 14 days past due when `invoiceReminders` is on; review-then-auto-send drafts whose timer elapsed (any edit clears it); holdback releases whose lien date passed → notify or auto-send.

**API** (`routes/invoices.ts`, Pro "invoicing"): `GET /invoices` (items + AR aging + stats), `GET /invoices/clients`, `GET/PUT/DELETE /invoices/:id` (drafts only are editable / discardable), `POST /invoices` (manual, job or client), `POST /jobs/:id/invoices` `{kind: deposit|term|final|holdback_release}`, `GET /jobs/:id/invoices`, `POST /invoices/:id/send|remind|void|credit-note|payments`, `DELETE …/payments/:pid`, `GET /invoices/:id/pdf`. `routes/public-invoices.ts` (IP rate-limited): `GET /i/:token` (first open flips sent → viewed), `GET /i/:token/pdf`. Job detail now returns `invoices` + `invoiceTotals`. `business-profile` accepts the new automation settings. The Fatture in Cloud mock `/crm/invoices/generate` is gone.

**Frontend**: `lib/invoices-api.ts`; `/dashboard/invoices` (stats, AR aging bar, filters, search, new-invoice dialog; Pro gate); `/dashboard/invoices/:id` (actions by state — edit / discard / send / resend / record payment / remind / credit note / void —, KPI strip, auto-send and lien-period banners, rendered document, customer link with copy, payments with remove, activity timeline); `/i/:token` public page in the invoice's language (balance, due date, e-Transfer address with copy, PDF, paid/void states); job page **Invoices** tab (`components/jobs/invoices-tab.tsx`: invoiced / collected / outstanding / still-to-invoice, billing plan with per-term state + one-click "Create invoice", manual invoice) and the job KPI "Invoiced" replaces the "Payments released" estimate; Overview payment list shows real invoice states; Documents tab lists sent invoices with PDF. Settings → Business: auto-send toggle, review window (never / 24 / 48 / 72 h), overdue reminders. Nav: Invoices (Pro) no longer "coming soon". EN + FR for every new string.

**Verified**: db/api-server/frontend typecheck, api-server + vite builds, `math.test.ts` unit run (ON holdback, QC GST/QST split, fixed-term back-out, final balance, lien days, status transitions, AR aging), browser smoke test against a mock API that uses the real renderer + PDF builder (list, detail, record payment → paid, edit draft → repriced + timer cleared, send → link, public page, job Invoices tab → create final invoice, settings). PDFs opened and checked. Not verified against a live DB, storage, Resend or the cron.

**Deferred**: change-order invoices as separate documents (signed COs are billed on the final invoice), Stripe/card payments, QuickBooks sync, per-line tax overrides (all lines taxable at the province rate), multi-currency, statement-of-account emails, holidays in the lien-period calculation.

### Phase 5 — Dashboards & assistant ✅ (built 2026-09-13, migration 0006 applied)

**Schema** (`lib/db/src/schema/assistant.ts`): `assistant_conversations` (one per user × job, or company-wide when `project_id` is null), `assistant_messages` (user / assistant with `tool_calls` / tool with `tool_call_id`), `assistant_proposals` (kind cost_entry | milestone_update | task | invoice | send_invoice | record_payment, summary, validated payload, status pending | confirmed | dismissed | failed, result entity, error). Migration `lib/db/drizzle/0006_phase5_assistant.sql` (additive, idempotent).

**Analytics** (`artifacts/api-server/src/analytics/`): `math.ts` is pure and unit-run in `math.test.ts` — `monthlySeries` (invoiced by issue date with credit notes deducted, cash collected excluding credit-note applications, confirmed costs, margin %), `budgetVsActual` per category (confirmed vs pending), `costCurve` (weekly cumulative actual / straight-line planned / invoiced / collected), `scheduleVariance` (slip days per milestone: finished late, running late, not started late; `daysBehind` = worst open slip), `earnedValue` (progress % × pre-tax value, billing gap vs invoiced pre-tax, CPI = budget earned ÷ actual, projected final cost = actual ÷ progress once ≥10 %, projected margin), `cashFlowForecast` (8 weeks: overdue balances in week 0, invoices by due week, milestone payment terms not yet invoiced shifted by the job's slip, remaining budgets spread over the remaining schedule, cumulative net), `jobRisks` (flags over_budget / budget_burn / behind_schedule / overdue_invoices / billing_gap / unbilled_completion with a score). `service.ts` loads rows and exposes `jobAnalytics(project)` and `companyAnalytics(userId, {months})`; job pre-tax value comes from the contract subtotal (+ change orders backed out of tax). Routes (`routes/analytics.ts`): `GET /jobs/:id/analytics` (jobs feature) and `GET /analytics/company?months=` (analytics_pro, Elite).

**Assistant** (`artifacts/api-server/src/assistant/`): `tools.ts` — OpenAI function-calling definitions and executors. Read tools: `get_job_summary`, `list_jobs`, `list_costs`, `list_invoices`, `list_time_entries`, `get_schedule_risks`, `get_company_overview` (all scoped to the user; amounts in CAD dollars for the model). Propose tools (`propose_cost_entry`, `propose_milestone_update`, `propose_task`, `propose_invoice`, `propose_send_invoice`, `propose_record_payment`) validate the arguments against the user's data (job/milestone/invoice ownership, tax split from the province, balances, state checks) and only create an `assistant_proposals` row — the tool result tells the model the user must confirm. `service.ts` runs a turn: system prompt (company, province, today, language, the current job's id/status/value) + last 40 stored messages → up to 6 tool rounds on `gpt-4o` (→ gpt-oss-120b through the Groq shim), 45 s timeout, `tool_choice: none` on the last round; every message and proposal is persisted. `apply.ts` executes a confirmed proposal with the same logic the manual routes use (milestone status → `recomputeProgress` + `milestone.completed` automation; invoices → `draftDeposit/Milestone/Final/HoldbackReleaseInvoice`; `sendInvoice`; `recordPayment`; cost entries stamped `created_by = ai`), re-checks the underlying feature, and marks the proposal confirmed / failed (with the error) — proposals are single-use. Routes (`routes/assistant.ts`, assistant feature = Elite, 120 messages/h per user): `GET /assistant/conversation?projectId=`, `POST /assistant/conversations/:id/messages {content, language}`, `DELETE /assistant/conversations/:id`, `POST /assistant/proposals/:id/confirm|dismiss`.

**Frontend**: `components/charts.tsx` (fixed, CVD-validated series palette + card/legend helpers); job page **Overview** now opens with `components/jobs/overview-charts.tsx` — health strip (projected margin + CPI, schedule on-track / days behind with forecast end, work done not yet billed), budget vs actual by category (over-budget bars in red, pending stacked), cost & cash curve, schedule-slip list — and gains an **Assistant** tab (`components/assistant/assistant-panel.tsx`: suggestions, bubbles with light markdown, proposal cards with Confirm / Dismiss / Applied → Open link / Failed reason, "Start over"; Elite gate with upsell). `/dashboard/analytics` rewritten (`pages/dashboard/analytics.tsx`, EN + FR): 3 / 6 / 12-month selector, Elite **Business** section (invoiced / collected / costs / gross margin / outstanding / upcoming billings tiles, monthly invoiced-costs-collected chart with a table view, AR aging bars, 8-week cash-flow forecast, jobs needing attention with flag chips, margin by job table, jobs by status) above the existing **Quotes** section (now translated). New `/dashboard/assistant` page (company-wide conversation) and nav item. `lib/analytics-api.ts`, `lib/assistant-api.ts`. Sidebar Pro items now also show for Elite subscribers. EN + FR for every new string (133 keys).

**Verified**: db/api-server/frontend typecheck, api-server + vite builds, `analytics/math.test.ts` unit run (month bucketing, P&L rules, budget vs actual, cost curve, schedule variance, earned value, cash-flow buckets, risk flags), browser smoke test against a mock API (analytics page EN, job overview charts + assistant tab: suggestion → answer, typed request → proposal card → confirm → Applied, company assistant page in FR). **Not verified against a live DB or the real model** — the tool-calling loop needs a live run (Groq gpt-oss-120b supports tools; watch for argument formatting).

**Deploy**: `0006_phase5_assistant.sql` applied to Supabase on 2026-09-13 before pushing.

**Deferred**: streaming responses, per-message "looked up X" chips, proposal editing before confirm (dismiss + re-ask for now), holidays in the forecast, company-level `propose_*` for jobs the model has to pick (works via `job_id`, but no picker UI), dashboard home widgets from the new analytics.
