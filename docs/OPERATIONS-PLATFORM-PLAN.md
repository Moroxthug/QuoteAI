...
### Phase 91 — Seats, sign-up, access codes, a page for every person (2026-09-23)

Asked by the user straight after Phase 90, together with "connect to Stripe": the add-on prices are created by a script the owner runs (no Stripe key or connector in this session).

**Built**
- **Stripe add-on prices script** (`scripts: stripe-addon-prices`): extra company in a group ($29/month, $290/year) and extra seat ($15/month, $150/year — the per-seat price was not specified, so this is the default to review), idempotent by lookup key, once per Stripe mode. One add-on driver (`lib/subscriptionAddons.ts`) now serves both, and plan code skips every add-on item.
- **Sign-up**: two new onboarding steps — *your work* (trades, people, logins wanted, crews on site) and *your team* (checkout with the plan from the pricing page plus the extra logins, or invite by email / access codes when the company already has team accounts). Someone who joined a company and owns none is sent past onboarding instead of creating an empty company.
- **Seats**: included + paid extras; the Team page shows used/limit and a Logins dialog (+/−, prorated, never below what is in use); the webhook keeps it equal to the subscription.
- **Access codes**: made on Team (or at sign-up) for a role, shown once (copy, print), typed at `/join`; the person creates their own login and lands on their profile setup.
- **Every person's page**: `/dashboard/me` (photo, name, title, phone, about; numbers — quotes made and won, win rate, invoiced, jobs, contracts, hours when on the crew list — by month; history) and `/dashboard/people/:id` for teammates. Avatars on the team list and in the account button. `created_by_user_id` on quotes, invoices, jobs, contracts; audit rows name the real person.
- Migration `0053` (applied). Route matrix 465 → 478. 179 EN/FR strings. Runbook §29.

**Found**
1. The account button's initials changed from the name's first two letters to word initials ("NL"), and the screen-reader sweep flagged it: the visible text was no longer part of the button's name (voice control). Reverted for that button; word initials stay on avatars, which sit next to the visible name.
2. A redeemed code had its hash cleared, so typing it again said "no such code" instead of "already used". The hash now stays.
3. "Resend" on an unused access code would have emailed the placeholder address; refused now (409 `CODE_INVITE`).
4. The e2e expectation for a quote's value assumed pre-tax; the stored total includes HST, and the page reports what the quote says.

**Not done / deferred**
- **The prices exist only after the owner runs the script** (and pastes the ids into Vercel). Until then Logins and One bill say "not available".
- Attribution is forward-only (from 2026-09-23); history before that stays the company's.
- Onboarding steps 2 and 4 aren't URL-addressable, so the visual sweep covers step 1 only; the new steps were checked by typecheck and by reading, not by a screenshot.
- Per-person analytics count what a person *created*; who *sent* or *won* a quote is not tracked separately.
- Crew in the field still use the magic link (no seat) — by design; a crew member who is also a login is linked by email only.

**Verification**: `pnpm typecheck` · `pnpm lint` **0 errors** (69 warnings, pre-existing) · `pnpm knip` no new unused exports · `i18n-audit` **4653 = 4653**, 0 missing, 0 split · `env:inventory` **no problems** (2 new variables classified; add-on price ids now read literally so the scanner sees them) · migration `0053` applied, `schema-drift` **0 missing, 0 mismatched** (101 tables) · route matrix **478**, rules green (the code preview on the public allowlist; the code redeem and the three /me writes as permissionless with reasons) · unit **28 files / 191 tests** · e2e **phase91 4/4** (sign-up answers; seats: included, refused without a price or subscription, bought 3 → limit 5, not below the 3 in use; codes: seat check, loose typing, redeem once with its role, expired gives the seat back, only managers make them; profile: setup, photo (and a text file refused), jobs and quotes stamped with who made them, stats and history, teammate view inside the company and 404 outside, audit rows naming the person) · regression **phase90, security (IDOR sweep), team, billing, quotes, money, lifecycle, phase86, phase89, account, public-tokens, phase83 — all green** · `pnpm build` 438 pages · `qa:visual` on /join (empty and with a code), onboarding, team-invite, /dashboard/me (owner and a foreman's first visit), Team members, EN+FR × 1280/375: 0 overflow, 0 gutter, **0 axe serious/critical**, 0 screen-reader findings after the fix above, 0 raw keys; /dashboard/me at 1280, the foreman's setup at 375 and Team members at 1280 checked by eye.
