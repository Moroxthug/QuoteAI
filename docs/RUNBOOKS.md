...
**How it fits**:
- **QuickBooks follows the invoice.** Sent → a QBO *Invoice* (one line, the invoice title; pre-tax with the mapped tax code, or tax-included when the invoice's tax set is not mapped). Each payment recorded in QuoteAI → a QBO *Payment* applied to it (credit notes are not sent). A payment removed → deleted in QBO. A void → voided in QBO. Invoices paid before Phase 88 were sent as one *SalesReceipt*; those are left alone (`hasLegacyReceipt`).
