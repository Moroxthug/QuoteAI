...
const MANIFEST: Record<string, Entry> = {
  ...
  STRIPE_PRICE_YEARLY_STARTER: { kind: "feature", feature: "annual billing", note: "Phase 73: yearly Stripe price id for Starter — `pnpm --filter @workspace/scripts stripe-annual-prices` creates all three and prints them; unset = annual toggle says 'coming soon'" },
  ...
  STRIPE_PRICE_GROUP_COMPANY: { kind: "feature", feature: "one bill for a company group", note: "Phase 90: monthly Stripe price id for each extra company a group pays for (scripts: stripe-addon-prices)" },
  STRIPE_PRICE_GROUP_COMPANY_YEARLY: { kind: "feature", feature: "one bill for a company group", note: "Phase 90: the same, for a yearly subscription (items cannot mix intervals)" },
  STRIPE_PRICE_EXTRA_SEAT: { kind: "feature", feature: "extra seats", note: "Phase 91: monthly Stripe price id for each login beyond the plan's included seats (scripts: stripe-addon-prices)" },
  STRIPE_PRICE_EXTRA_SEAT_YEARLY: { kind: "feature", feature: "extra seats", note: "Phase 91: the same, for a yearly subscription" },
  ...
  TWILIO_ACCOUNT_SID: { kind: "feature", feature: "SMS", note: "Phase 74 — Twilio; without the three vars every send is logged as skipped/not_configured" },
  TWILIO_AUTH_TOKEN: { kind: "feature", feature: "SMS", note: "also signs the inbound webhook (/api/sms/webhook)" },
  TWILIO_FROM_NUMBER: { kind: "feature", feature: "SMS", note: "E.164 Canadian number, toll-free verified" },
  TWILIO_WEBHOOK_URL: { kind: "feature", feature: "SMS", note: "optional; the exact URL configured in Twilio when it differs from <base>/api/sms/webhook (signature is computed over it)" },
  TWILIO_STATUS_CALLBACK_URL: { kind: "feature", feature: "SMS", note: "optional delivery-status callback; unused by the app today" },
  VAPID_PUBLIC_KEY: { kind: "feature", feature: "Push", note: "Phase 77 — Web Push (VAPID); without the pair the notifications page says push is not set up and no push is attempted. Generate with pnpm --filter @workspace/scripts vapid-keys" },
  VAPID_PRIVATE_KEY: { kind: "feature", feature: "Push", note: "Sensitive; signs the VAPID JWT for every push" },
  VAPID_SUBJECT: { kind: "feature", feature: "Push", note: "optional mailto:/https: contact the push services may use; defaults to mailto:support@quoteai.ca" },
  ...
  VERCEL_TOKEN: { kind: "local", note: "Phase 98: ops:owner-check reads the project's variable types (Sensitive or not) with it; never set in Vercel" },
  VERCEL_PROJECT_ID: { kind: "local", note: "with VERCEL_TOKEN (ops:owner-check, env:inventory)" },
  VERCEL_TEAM_ID: { kind: "local", note: "with VERCEL_TOKEN when the project is under a team" },
