...
### Phase 69 — Operations & launch readiness (2026-09-21)

**Built**
- **Error tracking** — `lib/error-reporting` (`@workspace/error-reporting`): a dependency-free Sentry envelope client (DSN parsing, V8 + Gecko stack → frames, cause chains, debug-id lookup for source maps, `text/plain` POST so the browser never preflights; 12 unit tests). Chosen over the official SDKs on purpose: `@sentry/node` drags OpenTelemetry into the esbuild bundle that is the one Vercel function, and `@sentry/react` would add ~30 kB to the 60 kB public entry Phase 68 fought for. Gives up breadcrumbs/tracing; keeps what launch needs. Server: `src/lib/errorTracking.ts` (`captureException` / `captureMessage` / `flush`, environment = `VERCEL_ENV`, release = `VERCEL_GIT_COMMIT_SHA`) wired into the Express error handler (every 500, with route/method/actor, no bodies or cookies, awaited so the function cannot end first), `automation.ts` (every failed attempt, `fatal` when dead), the cron tick, and `process.on(unhandledRejection|uncaughtException)`. Browser: `src/lib/error-tracking.ts` (window `error` / `unhandledrejection`, the React `ErrorBoundary`; 10 events per page load, duplicates dropped, `keepalive`). Inert until `SENTRY_DSN` / `VITE_SENTRY_DSN` are set. **Source maps**: `scripts/sentry-sourcemaps.mjs` runs at the end of both builds — with `SENTRY_AUTH_TOKEN`/`SENTRY_ORG`/`SENTRY_PROJECT` it injects debug ids (`sentry-cli sourcemaps inject`), uploads, and deletes the frontend `.map` files from `dist/public`; without them it prints one line and exits 0 (verified on both builds). Vite emits hidden source maps only when the token is present. CSP `connect-src` gained `https://*.sentry.io`.
- **Alerts** — migration `0035_phase69_cron_ticks.sql` (applied to `quoteai`) + `cronTicksTable`: every `/api/cron/tick` writes a row (`ok`, `result`, `error`, `tookMs`; 90-day retention). `src/lib/ops.ts`: `opsHealth()` (stale when no successful tick within `CRON_STALE_AFTER_HOURS`, default 25 — the schedule is daily on Hobby, so the plan's "> 1 h" became "> schedule + 1 h"; `dead` runs in 7 days; `failed` runs whose retry is overdue), `sendOpsAlert()` (email to `OPS_ALERT_EMAIL` → `ADMIN_EMAIL`, plus a Sentry warning tagged `logger:ops` for a paging rule), `pingHeartbeat()` (`CRON_HEARTBEAT_URL`, dead-man switch). New routes: **`GET /api/healthz/ops`** (public, 30/min per IP, 200/503 + `problems[]` — what the external uptime monitor polls; counts and timestamps only), `GET /api/admin/ops` (health + last 30 ticks), `GET /api/admin/automations?status=` and `POST /api/admin/automations/:id/retry` (one attempt now, backoff and attempt cap bypassed — there was no way to retry a dead run before). The tick now reports `backlog` and emails the operator whenever it is non-zero; a throwing tick is recorded, reported `fatal`, emailed.
- **Env inventory** — `pnpm env:inventory` (`scripts/src/env-inventory.ts`): scans every `process.env.X` / `import.meta.env.VITE_X` in both apps + libs (95 names), classifies each against an in-script manifest (required / recommended / feature / deferred / legacy / build / platform / local, with a note), diffs against Vercel (REST API with `VERCEL_TOKEN`, or `--vercel-keys <file>` with a pasted key list), exit 1 on an unclassified name or a missing `required` one. Snapshot committed as `docs/ENV-INVENTORY.md`.
- **Backups** — `pnpm --filter @workspace/api-server ops:backup` (`scripts/backup.ts`): pure Node (this machine has no Docker or `pg_dump`, and `supabase db dump` needs Docker) — one repeatable-read snapshot, `COPY … TO STDOUT` per table → `tables/<t>.csv.gz`, every Storage object, `manifest.json` with row counts / sha256 / sequences / migration list / schema introspection, optional AES-256-GCM per file (`BACKUP_PASSPHRASE`). `ops:restore` (`scripts/restore.ts`): `--wipe` (rebuild schema from `lib/db/drizzle/*.sql`) or `--truncate`, schema check against the manifest, `COPY FROM STDIN` in FK order inside one transaction (`session_replication_role=replica` when the role allows), sequences, row-count verification; refuses the backup's own source and non-empty targets; `--storage` re-uploads; **`--verify`** checks a backup without any database (decrypt, gunzip, sha256, CSV row counts). `.github/workflows/backup.yml`: nightly 07:17 UTC, backup → verify → encrypted artifact (30 days); refuses to run unencrypted; skips green until `BACKUP_*` secrets exist.
- **Key rotation** — `ops:rotate-token-key` (`scripts/rotate-token-key.ts`): re-encrypts the 10 encrypted columns across 7 `*_connections` tables from `OLD_` to `NEW_TOKEN_ENCRYPTION_KEY`; dry run by default, idempotent (values already under the new key are skipped, so it can be re-run to close the deploy window), refuses to write when any value decrypts with neither key (`--skip-undecryptable` to override), `--user` to scope.
- **Runbooks** — `docs/RUNBOOKS.md`: where things are; Sentry setup; cron/automation failures (stale tick, dead runs, retry); Stripe webhook backlog + resync; OAuth token revoked; backups + restore (incl. restoring production itself); rotate `TOKEN_ENCRYPTION_KEY` (and the other secrets); deploy rollback + migrations; a quick-reference table for the rest. `.env.example` documents the new variables.
- Tests: `src/lib/errorTracking.test.ts` (12) and `src/e2e/ops.e2e.test.ts` (2: dead run → `/healthz/ops` 503 → tick records itself + alert email with the run's error → admin list/retry revives it → probe clears; rotation refuse/dry-run/apply/idempotent on the e2e tenant's own row). The Resend mock now captures `text` emails.

**Found**
1. **The Supabase project has no backups at all** — Free plan: no daily backups, no PITR. Until the nightly workflow's secrets are set, the only backup is the one taken by hand today. Decision for the user: set the four `BACKUP_*` GitHub secrets now (free), and/or move to Pro (daily backups, 7-day retention; PITR is an add-on).
2. Vercel production env: all 20 `required` variables present; **none of the launch-recommended ones** — `SENTRY_DSN`/`VITE_SENTRY_DSN`, `OPS_ALERT_EMAIL`, `CRON_HEARTBEAT_URL`, `RESEND_WEBHOOK_SECRET` (the Resend webhook 500s and `email_events` stays empty), `POSTHOG_KEY`/`VITE_POSTHOG_KEY` (no product analytics in prod). Feature vars absent as expected: WhatsApp (8), Meta Lead Ads (4), Gmail send (3), Search Console (2); every deferred partner var absent by design. `STRIPE_CONNECT_WEBHOOK_SECRET`, `GOOGLE_CALENDAR_CLIENT_SECRET`, `QUICKBOOKS_CLIENT_SECRET` were created as plain Encrypted, not Sensitive (Vercel flags `readable-secret`) — recreate as Sensitive.
3. There was **no way to alert on or retry a dead automation run** and no record of whether the cron had fired at all; a dead run silently dropped a customer-facing side effect (contract draft, invoice, follow-up) forever.
4. With the daily schedule, the retry backoff (5 min … 24 h, 5 attempts) really means one attempt per day for five days — documented in the runbook; move to hourly on Pro and set `CRON_STALE_AFTER_HOURS=2`.
5. `TOKEN_ENCRYPTION_KEY` is not recoverable from Vercel (Sensitive) and is not in `.env.staging` — the rotation dry run against production correctly reported the two live token rows as undecryptable with a wrong key and wrote nothing. The real key must live in the password manager (runbook §6).
6. Production Storage buckets are empty (the one real contract was never finalised) — the backup's Storage half ran, but against nothing.

**Not done / needs the user**
- **Restore rehearsal into a second project**: the backup (77 tables, 83 rows) and its encrypted round-trip were verified with `--verify`; the loader itself has only been exercised in code. The one candidate target is the unused July project `cynlsphsuxrnctsxcmve` (eu-west-1) — the session's permission classifier blocked re-pointing the Supabase CLI at it to obtain credentials. To run it: get that project's pooler `DATABASE_URL` (dashboard → Database → reset password), then `pnpm --filter @workspace/api-server ops:restore --from .backups/rehearsal --target "<url>" --yes --wipe` and `DATABASE_URL="<url>" pnpm --filter @workspace/db schema-drift`. Or delete the July project and use its slot for the staging project Phase 63 still wants.
- Setting the recommended env vars, the GitHub backup secrets, the uptime monitor on `/api/healthz/ops`, and the Sentry project/alert rules (runbook §1, §2, §5).
- Optional integrations still log at `error` when unconfigured (Phase 66 P3) — left; nothing captures log lines, so it is Vercel-log noise only.

**Verification**: `pnpm typecheck` (root) · `pnpm lint` 0 errors (69 warnings) · `pnpm knip` (warn-level only) · unit 12 files / 55 tests · e2e 8 files / 61 tests green against `quoteai` · both apps built (sourcemap step skips without a token) · `validate-prerender` 415/415 · `docs/ROUTE-MATRIX.md` regenerated (346 routes) · migration 0035 applied · `pnpm env:inventory` against the live Vercel key list · `ops:backup` + `ops:restore --verify` plain and encrypted.

### Phase 70 — Human gate: help centre + go/no-go packet (2026-09-21)

Phase 70 has no code items by definition; this pass built the two things the assistant *could* deliver so the human review has something to review.

**Built**
- **Help centre** — `/help/` + ten guides at `/help/<slug>/` (`artifacts/quote-ai/src/data/help-articles.ts`, EN + FR side by side like the contract templates; rendered as structured blocks, no HTML injection): getting started (profile, taxes, licence, e-Transfer, plans), create a quote (text/voice/photo, catalog, Good/Better/Best, PDF templates), send & accept (link/email, `/p`, follow-ups 2/5/10), contracts & e-signature (province templates, OTP, audit certificate, reminders 3/7/14), jobs (setup review, milestones, calendar, change orders, holdback release), invoices & payments (e-Transfer, Stripe card, record payment, reminders, void), costs/receipts/time (receipt AI, worker links 6 months, geofence, payroll CSV), team & roles (admin/office/foreman/viewer, seats 2/5), leads & follow-ups (sources, pipeline, CASL, review request 3 days after completion), integrations & imports (Gmail, calendars, QuickBooks/Wave, Stripe Connect, CSV/PDF import, API/Zapier). Every number and behaviour was checked against the code before being written (follow-up cadence, reminder days, OTP rules, review-request gate, seat counts, photo limits); three claims that could not be verified (Google sign-up, account deletion, per-job review opt-out) were removed. Pages: `src/pages/help/index.tsx`, `src/pages/help/[slug].tsx`; `help.*` + `sitemap.helpCenter` keys in both languages; `.help-note` callout style. Wired everywhere a public page needs to be: `PUBLIC_ROUTES` (`HELP`), `App.tsx`, `entry-server.tsx` `SSR_PAGES` (index + 10 articles), `main.tsx` `SSR_PAGE_RE`, `prerender-seo.ts` (TechArticle/CollectionPage JSON-LD + breadcrumbs, 428 pages now), `generate-sitemap.ts` (article `updatedAt` as lastmod), the site-map page, the footer ("Help centre" is now a link; the mailto "Contact support" modal keeps its own button). The support bot's system prompt now lists the ten guides with their topics and is told to link `https://quoteai.ca/help/<slug>/` rather than invent behaviour, and to answer in the visitor's language.
- **`docs/LAUNCH-GO-NO-GO.md`** — the agenda for the human gate: §1 ten recommended blockers with state, exact action, owner (backups secrets, restore rehearsal, template legal review, ToS/privacy + registered entity, staging project, Sentry/heartbeat/uptime, QC P1s, readable Vercel secrets, `TOKEN_ENCRYPTION_KEY` escrow); §2 the legal packet (what to hand the lawyer, per province, plus e-sign evidence, CRA invoices, CASL, ToS/privacy incl. the missing erasure/portability, help-centre wording); §3 recommended-not-blocking (PostHog, Resend webhook, hourly cron, pentest, real-device pass, live AI pass, WhatsApp templates, OAuth registrations); §4 accepted post-launch list; §5 an empty decision record — when it has a row, Phase 70 is closed.

**Verification**: `pnpm typecheck` (root) · `pnpm lint` 0 errors (69 warnings) · `pnpm knip` clean for the new files · `i18n-audit` clean (0 missing keys, split 0/0) · unit 12 files / 55 tests (route matrix regenerated — support.ts line numbers only) · quote-ai build + prerender 428 pages · `validate-prerender` 426/426 · `validate-sitemap` 428/428 · `/help/` and `/help/invoices-and-getting-paid/` checked in the browser (FR toggle, TOC, note callout, next link, footer link; the only console errors were `/api/auth/get-session` 500s from a dev server with no API — unrelated).

**Not done / needs the user** — everything in `LAUNCH-GO-NO-GO.md` §1-§3. The e2e suite was not re-run (no route or data change; only a prompt string in `support.ts`).
